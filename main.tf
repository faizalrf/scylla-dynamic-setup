...
resource "google_compute_instance" "scylla-node" {
  ...
  # Dynamically attach NVMe disks to the Instance, disk count should be (0, 1, 2, 4...)
  dynamic "scratch_disk" {
    for_each = range(var.nvme_disk_count)
    content {
      interface = "NVME"
    }
  }
...
# Open monitoring firewall, port 3000
resource "google_compute_firewall" "allow_monitoring" {
  name    = "allow-monitoring-${random_id.firewall_suffix.hex}"
  network = "default"
